...
fn tests(src_dir: &Path) -> Vec<Test> {
    ...
    // Look for test files in the html5lib-tests directory we created
    let mut test_dir = src_dir.to_path_buf();
    test_dir.push("html5lib-tests/tokenizer");
    ...
    // Also check for custom tests
    let custom_test_dir = src_dir.join("custom-html5lib-tokenizer-tests");
    if custom_test_dir.exists() {
        if let Ok(entries) = std::fs::read_dir(custom_test_dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.extension() == Some(OsStr::new("test")) {
                    if let Ok(file) = std::fs::File::open(&path) {
