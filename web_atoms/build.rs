...
fn named_entities_to_phf(to: &Path) {
    let mut entities: BTreeMap<String, (u32, u32)> = entities::NAMED_ENTITIES
        .iter()
        .map(|(name, cp1, cp2)| {
            assert!(name.starts_with('&'));
            (name[1..].to_string(), (*cp1, *cp2))
        })
        .collect();

    // Add every missing prefix of those keys, mapping to NULL characters.
    let prefixes: Vec<_> = entities.keys().cloned().collect();
    for key in prefixes {
        for n in 1..key.len() {
            entities.entry(key[..n].to_string()).or_insert((0, 0));
        }
    }
    entities.insert("".to_string(), (0, 0));
